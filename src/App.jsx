        ...
        {isHome && (
          ...
          >
            <div style={{ width: '100%', paddingLeft: '40px', paddingRight: '40px', paddingTop: '16px', paddingBottom: '16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <div className="flex items-center gap-1 sm:gap-2 bg-[#0a0a0a]/60 backdrop-blur-md border border-white/10 px-2 py-1.5 rounded-full shadow-[0_4px_24px_rgba(0,0,0,0.4)]">
                <button onClick={() => { setIsShopOpen(false); setIsInquiryOpen(false); navigate('/'); }} className={`animate-fade-in hidden sm:block px-4 sm:px-6 py-2 rounded-full text-white text-sm sm:text-base font-bold tracking-wide transition-all duration-300 ${!isShopOpen && !isInquiryOpen && location.pathname === '/' ? 'bg-yellow-400/20' : 'hover:bg-white/10'}`} style={{ textShadow: '0 2px 10px rgba(0,0,0,1), 0 1px 4px rgba(0,0,0,0.8)', animationDelay: '0.1s' }}>Home</button>
                <button onClick={() => setIsShopOpen(true)} className={`animate-fade-in px-4 sm:px-6 py-2 rounded-full text-white text-xs sm:text-base font-bold tracking-wide transition-all duration-300 ${isShopOpen ? 'bg-yellow-400/20' : 'hover:bg-white/10'}`} style={{ textShadow: '0 2px 10px rgba(0,0,0,1), 0 1px 4px rgba(0,0,0,0.8)', animationDelay: '0.2s' }}>Shop</button>
                <button onClick={() => navigate('/my-cart')} className={`animate-fade-in px-4 sm:px-6 py-2 rounded-full text-white text-xs sm:text-base font-bold tracking-wide transition-all duration-300 ${location.pathname === '/my-cart' ? 'bg-yellow-400/20' : 'hover:bg-white/10'}`} style={{ textShadow: '0 2px 10px rgba(0,0,0,1), 0 1px 4px rgba(0,0,0,0.8)', animationDelay: '0.3s' }}>My Carts</button>
                <button onClick={() => setIsInquiryOpen(true)} className={`animate-fade-in px-4 sm:px-6 py-2 rounded-full text-white text-xs sm:text-base font-bold tracking-wide transition-all duration-300 ${isInquiryOpen ? 'bg-yellow-400/20' : 'hover:bg-white/10'}`} style={{ textShadow: '0 2px 10px rgba(0,0,0,1), 0 1px 4px rgba(0,0,0,0.8)', animationDelay: '0.4s' }}>Inquiry</button>
              </div>
            </div>
          </nav>
        )}
